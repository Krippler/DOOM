...
//
// The engine reads controllers itself (linuxdoom-1.10/i_pad.c): what each
// button does is set in the game, under Options -> Setup -> Controller, and
// its sticks walk and turn -- the same in this container and in a desktop
// build. What the engine cannot do from inside a container is see the pad,
// which is plugged into the machine running this browser. So this file reads
// it through the Gamepad API and sends its state across, and that is all it
// does: no bindings, no stick maths, nothing to configure here.
//
// It replaces a page-side translation into X keysyms, which could only press
// keys the X server would pass on (Return, Tab and the F keys never arrived),
// could not tell a menu from the game, and had to be told about every key
// rebound in the game.
//
...
// on. Two messages, little-endian, as i_pad.c reads them:
//
...
export class DoomPadBridge {
  constructor({ url, onChange } = {}) {
    ...
    this.rumbles = 0;               // vibrations played, for the log
  }
  ...
  _state(pad, active) {
    ...
    // The triggers go as their own analog values as well; i_pad.c decides
  ...
  // Whether this pad can vibrate at all, as far as the browser admits.
  canRumble() {
    const pad = this._pad;
  ...
  _connect() {
    ...
    // The engine restarts after a crash or a quit, and its end of this goes
    // with it; the next poll connects again.
    s.onclose = s.onerror = () => {
